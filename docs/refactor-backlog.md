...
#### Senku striped-ingestion memory and hashing

- [ ] 102. Eliminate pathological `HashMap` collision trees and the redundant
  frozen-batch copy from parallel Senku ingestion (Risk: HIGH). Owner: `engine`
  Senku ingestion and `benchmarks`. Problem: `SenkuIngestor` currently selects
  one of 32 mutation stripes with the same spread low bits used by JDK
  `HashMap` bucket selection. Every map therefore receives keys whose bucket
  indices share five fixed low bits, leaving only one thirty-second of its
  allocated buckets usable and forcing collision bins into red-black trees.
  During a filesystem-backed `Long`/`NullValue` peg-solitaire workload, JFR
  attributed `29.96%` of execution samples to `HashMap.getNode`, `4.20%` to
  `HashMap.TreeNode.find`, and `10.76%` of allocation pressure to
  `HashMap.newTreeNode`. The flush path then calls `combine(...)`, allocating a
  third complete `HashMap` while an active batch and a detached flushing batch
  are both live. The same profile recorded `55.78%` of allocation pressure in
  boxed `Long` creation, a roughly `7.3 GiB` resident process, `17` full
  collections, and about `124 seconds` of stop-the-world GC over an
  approximately eighteen-minute run. Outcome: concurrent ingestion distributes
  keys independently of the inner map's bucket bits, and `SenkuFlushWriter`
  consumes detached stripe maps without cloning every mapping into another
  `HashMap`. Guardrails: preserve atomic duplicate merging, comparator-equal-key
  routing, lifecycle and failure propagation, bounded one-detached-batch
  flushing, backpressure, flush-generation ordering, public APIs, builder
  defaults, and the persisted Senku layout. Do not weaken correctness to reach
  a throughput target, introduce an unbounded flush queue, or retain detached
  maps after their flush completes. Validation: items `102.1` through `102.3`,
  matching unit and integration tests, `mvn clean verify`, documentation
  navigation and strict builds, and a clean `git diff --check`.

  - [ ] 102.1 Make mutation-stripe selection independent of JDK `HashMap`
    bucket selection (Risk: HIGH). Owner: `SenkuIngestor` construction and
    routing. Outcome: a deterministic avalanche-quality hash selects the 32
    mutation stripes without reusing the exact low bits that the inner
    `HashMap` uses for its bucket index. The existing configured Senku shard
    hash may be reused internally when its contract is sufficient; no new
    public builder option is required. Equal keys must always select the same
    mutation stripe, while distinct keys on different stripes remain eligible
    for concurrent mutation. Guardrails: stripe routing is an in-memory concern
    and must not change persistent shard IDs, sorted output, duplicate merge
    results, flush thresholds, or reopen compatibility. Validation: direct
    deterministic tests for equal keys, duplicate concurrent puts, negative
    and extreme hash values, rotation races, and at least one adversarial key
    population whose JDK-spread low five bits are biased. Across one million
    representative biased `long` board-state keys, the mutation-stripe test
    must keep maximum/minimum population below `1.10`; the old selector must be
    demonstrated to fail that fixture or an equivalent collision fixture.

  - [ ] 102.2 Flush detached stripe maps without a complete combined-map copy
    (Risk: HIGH). Owner: `SenkuIngestor` and `SenkuFlushWriter`. Outcome: the
    flush writer accepts a read-only aggregate view or iterable collection of
    the detached maps and performs shard counting, ordering, sorting, and
    persistence directly from those maps. Peak ingestion state may contain one
    active striped batch, one detached striped batch, and the existing
    `O(entryCount)` ordering references, but no third `HashMap` containing one
    node per detached entry. Guardrails: do not mutate detached maps while they
    are being written; publish the manifest last; release every detached map
    after success or terminal failure; retain exact duplicate semantics within
    the active batch; and preserve incomplete-flush failure behavior. Validation:
    component tests cover empty stripes, one and many non-empty stripes,
    duplicates, exact and overshot rotation thresholds, multi-part output,
    write failure cleanup, a new rotation while the prior batch flushes, and
    finalization waiting for a detached flush. A test-visible seam or allocation
    profile must prove that flushing does not allocate a second set of
    `HashMap.Node` or `HashMap.TreeNode` objects for the detached entries.

  - [ ] 102.3 Establish a production-shaped Senku ingestion performance gate
    (Risk: MEDIUM). Owner: Senku JMH benchmarks and development documentation.
    Outcome: compare the accepted implementation with commit `c29ba8fd5` using
    disjoint eight-thread `Long`/null-value input, biased peg-board-style keys,
    rotation and overlapping filesystem-backed flushes, and a heap budget that
    exposes peak live memory. Record throughput, allocation per operation, peak
    heap/RSS, young/full collection counts and pause time, flush cadence, and
    p50/p95/p99 put latency including rotations. Guardrails: use multiple forks
    on a quiet host, retain raw artifacts, separate in-memory lock throughput
    from filesystem flush throughput, and do not claim improvement from a
    single point estimate. Acceptance: no material `HashMap.TreeNode` hot path
    for a well-distributed key equality contract, no full collection caused by
    the removed combined-map copy in the bounded reference profile, lower peak
    live memory and allocation per operation than the baseline, and no
    statistically supported throughput or tail-latency regression. Update
    `docs/development/senku-index.md` and its ingestion diagram to describe the
    accepted striped-map and detached-flush architecture rather than the stale
    single-map stop-the-world baseline.
