...
[Bulk Ingestion with Senku](../how-to-use/senku-index.md) for a runnable example.
...
Each lifecycle handle exposes the operations appropriate to its state. The
following API excerpt omits Javadoc and method bodies.
...
| `shardHashFunction` | Required for `create()`; `createLongSet(hash)` supplies its own primitive hash | Non-null, deterministic; equal keys must have the same hash | `IllegalArgumentException` if missing or null | No |
...
- `maxInMemoryEntries`: the approximate distinct-key count that requests batch
  rotation; duplicate puts merged into an existing key do not increase it. The
  sampled count can lag the exact count by less than 25 percent of the limit.
...
- A ready-stream storage or decoding failure is reported at the failing call.
  Close the stream and handle; the API does not repair damaged persisted data.
- A `SenkuWriting` in `ERROR` rejects ingestion and cannot finalize a valid
  index. `finishWriting()` reports the recorded first failure. Minimal failure
  shutdown stops owned work and releases the root `FileLock`, but performs no
...
The coordinator permits exactly one active L0 batch. Only the
...
1. Acquire the lifecycle writing lock, close ingestion admission, wait for an
   active detached flush, and flush the remaining striped batch.
2. Transition the lifecycle to `FINISHING` and release the lifecycle writing
   lock after the flush is committed.
...
- Ready cursor advancement does not acquire the handle lock. Callers must
  serialize consumption with stream and ready close.
...
- `maxInMemoryEntries` limits entry count rather than bytes. The same setting can
  ...
  value types. It also does not bound peak flush memory: one active batch, one
...
- With readers kept open for speed, maintenance can require approximately
  ...
  resource-safe configuration. Senku leaves these
...
- Before `READY`, Senku provides no durability, crash recovery, or resume
  guarantee. WAL support is not provided.
...
The builder is one flat object. It validates inputs and constructs Senku
...
damage is an accepted limitation unless the filter decision is
reopened.
...
1. The first reporter records the primary failure for coordinator use and wakes
   ...
   to `ERROR`, tells the ingestor to close admission and clear `paused`, and
   signals callers waiting on `ingestionMayProceed`. A detached flush already
   in progress finishes or reports its own failure. Later exceptions never
   replace the internally retained failure.
...
5. The coordinator waits until the worker executor terminates and any
   ...
   invoke `FileLock.unlock()`, shut down its scheduled executor, and count down
   `maintenanceFinished`.
6. A `finishWriting()` caller that had already begun the terminal operation
   ...
   rethrows the recorded first `IndexException` with its original cause chain.
   A later finish call also reports the recorded failure; a later put receives
   only the generic lifecycle rejection.
7. Senku performs no filesystem traversal or explicit deletion. Artifacts may
   ...
   root-lock-release failure is retained on the primary failure. The writing
   API has no public cleanup retry operation.
...
- An incorrect flush `recordCount` that differs by complete pages and still ends
  ...
  rehashing every decoded key. This is a current format limitation.
...
- [Bulk Ingestion with Senku](../how-to-use/senku-index.md) for application integration.
- [Refactor Backlog](../refactor-backlog.md) for completed implementation work.
- [Senku implementation issue](https://github.com/jajir/HestiaStore/issues/347).
