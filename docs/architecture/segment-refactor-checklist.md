...
1) [ ] Confirm contract is final in `docs/architecture/segment-concurency.md`.
2) [ ] Inventory current segment classes and call sites.
3) [ ] Create `Segment` public interface (API parity with current segment).
4) [ ] Extract current single-threaded logic into `SegmentCore`.
5) [ ] Add `SegmentImpl` that delegates to `SegmentCore` (no concurrency logic yet).
6) [ ] Switch call sites to depend on `Segment` interface (use adapter if needed).
7) [ ] Implement `SegmentStateMachine` with serialized transitions.
