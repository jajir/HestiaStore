...
final class WalBackpressureCoordinator<K, V> {
    ...
    private final WalCheckpointMaintenance checkpointMaintenance;
    ...
    WalBackpressureCoordinator(
        ...
        this.checkpointMaintenance = Vldtn.requireNonNull(
    ...
    void enforceIfNeeded() {
        ...
        while (walRuntime.isRetentionPressure()) {
            checkpointAttempts++;
            checkpointMaintenance.flushAndWait();
    ...
    private void logWalRetentionPressureCleared(final long startNanos,
        ...
        LOGGER.info(
                ...
                walRuntime.retainedBytes(),
                conf.wal().getMaxBytesBeforeForcedCheckpoint(),
                checkpointAttempts, Math.max(0L, elapsedMillis));
    }
