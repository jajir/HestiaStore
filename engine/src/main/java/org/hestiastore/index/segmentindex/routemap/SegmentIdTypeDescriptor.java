 ...
 *
 */
final class SegmentIdTypeDescriptor implements TypeDescriptor<SegmentId> {
    ...
    public Comparator<SegmentId> getComparator() {
        return (segId1, segId2) -> Integer.compare(segId2.getId(),
                segId1.getId());
    }
    ...
    public TypeEncoder<SegmentId> getTypeEncoder() {
        return (segmentId, reusableBuffer) -> tdi.getTypeEncoder()
                .encode(segmentId.getId(), reusableBuffer);
    }
