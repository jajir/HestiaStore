...
public class IndexConfigurationManager<K, V> {
    ...
    public IndexConfiguration<K, V> applyDefaults(
        ...
        final int effectiveSegmentWriteCacheKeyLimit =
                applySegmentWriteCacheDefaults(conf, builder,
                        effectiveMaxNumberOfKeysInSegmentCache);
        applyMaintenanceWriteCacheDefaults(conf, builder,
                effectiveSegmentWriteCacheKeyLimit);
    ...
    private int applySegmentWriteCacheDefaults(
            final IndexConfiguration<K, V> conf,
            final IndexConfigurationBuilder<K, V> builder,
            final int effectiveMaxNumberOfKeysInSegmentCache) {
        if (conf.writePath().segmentWriteCacheKeyLimit() == null) {
            final int effectiveSegmentWriteCacheKeyLimit = Math.max(1,
                    effectiveMaxNumberOfKeysInSegmentCache / 2);
            builder.writePath(writePath -> writePath.segmentWriteCacheKeyLimit(
                    effectiveSegmentWriteCacheKeyLimit));
            return effectiveSegmentWriteCacheKeyLimit;
        }
        return conf.writePath().segmentWriteCacheKeyLimit();
    }

    private void applyMaintenanceWriteCacheDefaults(
            final IndexConfiguration<K, V> conf,
            final IndexConfigurationBuilder<K, V> builder,
            final int effectiveSegmentWriteCacheKeyLimit) {
        if (conf.writePath()
                .segmentWriteCacheKeyLimitDuringMaintenance() == null) {
            final int effectiveMaintenanceWriteCacheKeyLimit = Math.max(
                    (int) Math
                            .ceil(effectiveSegmentWriteCacheKeyLimit * 1.4),
                    effectiveSegmentWriteCacheKeyLimit + 1);
    ...
    private boolean applyBasicOverrides(
        ...
        dirty |= applyIf(
                isSegmentWriteCacheKeyLimitOverridden(storedConf, indexConf),
                () -> builder.writePath(writePath -> writePath
                        .segmentWriteCacheKeyLimit(indexConf.writePath()
                                .segmentWriteCacheKeyLimit())));
        dirty |= applyIf(
                isMaintenanceWriteCacheKeyLimitOverridden(storedConf,
                        indexConf),
                () -> builder.writePath(writePath -> writePath
    ...
    private boolean isSegmentWriteCacheKeyLimitOverridden(
            final IndexConfiguration<K, V> storedConf,
            final IndexConfiguration<K, V> indexConf) {
    ...
    private boolean isMaintenanceWriteCacheKeyLimitOverridden(
            final IndexConfiguration<K, V> storedConf,
            final IndexConfiguration<K, V> indexConf) {
    ...
    private void validateSegmentLimits(final IndexConfiguration<K, V> conf) {
        ...
        if (conf.writePath().segmentSplitKeyThreshold() < 1) {
            throw new IllegalArgumentException(
                    "Segment split key threshold must be at least 1.");
        }
    ...
    private void validateWritePathLimits(final IndexConfiguration<K, V> conf) {
        Vldtn.requireNonNull(conf.writePath().segmentWriteCacheKeyLimit(),
                "segmentWriteCacheKeyLimit");
        if (conf.writePath().segmentWriteCacheKeyLimit() < 1) {
            throw new IllegalArgumentException(
                    "Segment write-cache key limit must be at least 1.");
        }
        ...
        if (effectiveMaxDuringMaintenance <= segmentWriteCacheKeyLimit) {
            throw new IllegalArgumentException(
                    "Segment write-cache key limit during maintenance must be greater than segment write-cache key limit.");
        }
    }
