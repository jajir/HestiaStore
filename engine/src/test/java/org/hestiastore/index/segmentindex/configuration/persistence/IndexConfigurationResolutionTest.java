...
class IndexConfigurationResolutionTest {

    @Test
    void ofStoresConfigurationAndWriteFlag() {
        final EffectiveIndexConfiguration<Integer, String> configuration =
                effective(buildConfiguration());

        final IndexConfigurationResolution<Integer, String> resolution =
                IndexConfigurationResolution.of(configuration, true);

        assertSame(configuration, resolution.configuration());
        assertTrue(resolution.writeRequired());
    }

    @Test
    void ofRejectsNullConfiguration() {
        assertThrows(IllegalArgumentException.class,
                () -> IndexConfigurationResolution.of(null, false));
    }

    private static IndexConfiguration<Integer, String> buildConfiguration() {
