...
- Flag parallel access paths to the same mutable runtime state as a design finding. If a feature path can bypass the package or domain service that owns an invariant and instead reaches the underlying map, registry, topology, storage, or lifecycle object directly, call out the inconsistency risk. Prefer one authoritative coordinator per invariant, or a narrow package-private gateway that owns all access to the shared state.
...
- Treat lifecycle ownership hidden behind generic callbacks as a design finding. When an open, close, recover, checkpoint, rollback, cleanup, or teardown action represents ownership of real resources, prefer a named lifecycle collaborator such as a package-private interface or domain resource object over `Runnable`, method-reference callbacks, or callback fields.
